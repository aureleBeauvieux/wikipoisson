...
const createCommentaire = async(note, commentaire, date, validation, user_id, id_espece) => {
    const sql = `
    ...
    try {
        result = await query(sql, [note, commentaire, date, validation, user_id, id_espece]);
    } catch (e) {
        error = e.message;
    } finally {
        return { error, result };
...
const updateCommentaire = async(id_commentaire, note, commentaire, date, validation, user_id, id_espece) => {
    const sql = `
        UPDATE commentaire
    ...
    try {
        result = await query(sql, [note, commentaire, date, validation, user_id, id_espece, id_commentaire]);
    } catch (e) {
        error = e.message;
    } finally {
        return { error, result };
