...
const createCommentaire = async(req, res) => {
    ...
    const response = await CommentaireDB.createCommentaire(note, commentaire, date, validation, user_id, id_espece);
...
const updateCommentaire = async(req, res) => {
    // Extraction des données de la requête
    const { note, commentaire, date, validation, user_id, id_espece, id_commentaire } = req.body;

    // Appel à la fonction de la base de données pour mettre à jour un commentaire
    const response = await CommentaireDB.updateCommentaire(note, commentaire, date, validation, user_id, id_espece, id_commentaire);
    ...
    return res.status(200).json({ message: `Le commentaire numéro ${id_commentaire} a été modifié` });
};
...
const updateValidation = async(req, res) => {
    ...
    const response = await ContributionDB.updateValidation(id_commentaire, validation);

    if (response.error) {
        return res.status(500).json({ message: response.error });
    }

    return res.status(200).json({ message: `La validation du commentaire ${id_commentaire} a été mise à jour` });
};
