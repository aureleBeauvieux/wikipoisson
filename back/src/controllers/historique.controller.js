...
const createHistorique = async(req, res) => {
    ...
    const { recherches, date, user_id } = req.body;

    // Appel à la fonction de la base de données pour créer un historique
    const response = await HistoriqueDB.createHistorique(recherches, date, user_id);
...
const readOneHistorique = async(req, res) => {
    ...
    const historique = {
        id_historique,
        recherches: result[0].recherches,
        date: result[0].date,
...
const updateHistorique = async(req, res) => {
    // Extraction des données de la requête
    const { recherches, date, user_id, id_historique } = req.body;

    // Appel à la fonction de la base de données pour mettre à jour un historique
    const response = await HistoriqueDB.updateHistorique(recherches, date, user_id, id_historique);
    ...
    return res.status(200).json({ message: `L'historique numéro ${id_historique} a été modifié` });
};
